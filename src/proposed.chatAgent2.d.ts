...
declare module "vscode" {
  ...
  export interface ChatAgentSubCommandProvider {
    ...
    ): ProviderResult<ChatAgentSubCommand[]>;
  }
  ...
  /**
   * Will be invoked once after each request to get suggested followup questions to show the user. The user can click
   * the followup to send it to the chat.
   */
  export interface FollowupProvider<TResult extends ChatAgentResult2> {
    /**
     *
     * @param result The same instance of the result object that was returned by the chat agent, and it can be extended
     * with arbitrary properties if needed.
     * @param token A cancellation token.
     */
    provideFollowups(
      result: TResult,
  ...
  export interface ChatAgentRequest {
    /**
     * The prompt entered by the user. The {@link ChatAgent2.name name} of the agent or the {@link
     * ChatAgentSubCommand.name subCommand}
    ...
    /**
     * The {@link ChatAgentSubCommand subCommand} that was selected for this request. It is guaranteed that the passed
     * subCommand is an instance that was previously returned from the {@link
     * ChatAgentSubCommandProvider.provideSubCommands subCommand provider}.
  ...
  /**
   * Is displayed in the UI to communicate steps of progress to the user. Should be used when the agent may be slow to
   * respond, e.g. due to doing extra work before sending the actual request to the LLM.
   */
  ...
  /**
   * Indicates a piece of content that was used by the chat agent while processing the request. Will be displayed to the
   * user.
   */
  ...
  /**
   * A piece of the chat response's content. Will be merged with other progress pieces as needed, and rendered as
   * markdown.
   */
  ...
  /**
   * Represents a piece of the chat response's content that is resolved asynchronously. It is rendered immediately with
   * a placeholder,
  ...
  export interface ChatAgentFileTreeData {
    ...
    // TODO@API don't use uri but just names? This API allows to to build nonsense trees where the data structure
    // doesn't match the uris
    // path-structure.
    uri: Uri;

    /**
     * The type of this node. Defaults to {@link FileType.Directory} if it has {@link ChatAgentFileTreeData.children
     * children}.
     */
  ...
  export interface ChatVariableValue {
    /**
     * The detail level of this chat variable value. If possible, variable resolvers should try to offer shorter values
     * that will consume fewer tokens in an LLM prompt.
     */
    level: ChatVariableLevel;

    /**
     * The variable's value, which can be included in an LLM prompt as-is, or the chat agent may decide to read the
     * value and do something else with it.
     */
    value: string | Uri;
